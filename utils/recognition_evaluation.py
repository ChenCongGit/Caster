...
class RecognitionEvaluation(object):
  ...
  def add_single_image_recognition_info(self, image_key, recognition_text, groundtruth_text):
    ...
    """
    if image_key in list(self.image_keys):
  ...
  def add_batch_images_recognition_info(self, batch_image_key, batch_recognition_text, batch_groundtruth_text):
    """
    """
    for i in range(batch_image_key.shape[0]):
      image = batch_image_key[i]
      if image in list(self.image_keys):
        # logging.warning('{} already evaluated'.format(image))
        continue
      self.image_keys.add(image)
      self.all_recognition_text.append(batch_recognition_text[i].decode('utf-8'))
      self.all_groundtruth_text.append(batch_groundtruth_text[i].decode('utf-8'))
